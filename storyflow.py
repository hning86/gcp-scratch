...
class StoryFlowAgent(BaseAgent):
    ...
    async def _run_async_impl(
        ...
        logger.info(f"[{self.name}] Tone check result: {repr(tone_check_result)}")
...
async def call_agent_async(user_input_topic: str):
    ...
    story_content = final_session.state.get("current_story")
    if story_content:
        with open("story.md", "w") as f:
            f.write(story_content)
        print("SUCCESS: Final story saved to story.md\n")
    else:
        print("WARNING: No story found in the final session state.\n")

# --- Run the Agent ---
# Note: In Colab, you can directly use 'await' at the top level.
# Since running this code as a standalone Python script, we must use asyncio.run()
